...
- **stealthy_headers**: If enabled (default), it creates and adds real browser headers. It also sets a Google referer header. Disabling it does not disable browser impersonation headers generated by `curl_cffi` when `impersonate` is enabled.
